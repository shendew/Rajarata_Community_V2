...
public class AddEventScreen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        image=findViewById(R.id.eventImg);
        title=findViewById(R.id.eventTitle);
        desc=findViewById(R.id.eventDesc);
        Button add=findViewById(R.id.eventAdd);
        ...
        add.setOnClickListener(view -> {
            ...
            if (!Ttitle.isEmpty() && Image_BACK!=null){
                ...
                StorageReference ref= FirebaseStorage.getInstance().getReference("EventImages").child(id+"jpg");
                            ...
                            ref.getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                                ...
                                public void onComplete(@NonNull Task<Uri> task) {
                                    if (task.isSuccessful()){
                                        ...
                                        Toast.makeText(AddEventScreen.this, "Failed to get file url", Toast.LENGTH_SHORT).show();
                                    }
                                }
                            });
                        }else{
                            Toast.makeText(AddEventScreen.this, "No Image FOound", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }else {
...
//        map.put("desc",tdesc);
        map.put("id",id);
        map.put("url",url);
        FirebaseDatabase.getInstance().getReference("Events").child(String.valueOf(id)).setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                loadingDialog.hideDialog();
                if (task.isSuccessful()){
                    Toast.makeText(AddEventScreen.this, "Event added Successfully", Toast.LENGTH_SHORT).show();
                    finish();
                }else {
                    Toast.makeText(AddEventScreen.this, "Something went wrong,please try again later", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode ==REQUEST_CODE_LOAD_IMAGE && resultCode == RESULT_OK){
            try {
                ...
                Glide.with(AddEventScreen.this).load(bitmap).centerCrop().into(image);
